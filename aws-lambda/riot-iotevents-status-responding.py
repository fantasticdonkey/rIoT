""" riot-iotevents-status-responding.py
AWS Lambda function to respond to IoT Events changing a tracker's state into
"responding" from any other state.  On entering this state, it does the following:

- Updates tracker's Shadow document in AWS IoT Core with the new state: responding.
...
def lambda_handler(event, context):
    ...
    SUBJECT = event["payload"]["detector"]["keyValue"] + " is responding"
    BODY_TEXT = (
        "Response received from tracker " + event["payload"]["detector"]["keyValue"]
    )
    BODY_HTML = """<html>
    <head></head>
    <body>"""
    event["payload"]["detector"]["keyValue"] + ": responses are being received from the tracker" """
    </body>
    </html>
    """
    CHARSET = "UTF-8"
