""" riot-iotevents-status-not-responding.py
AWS Lambda function to respond to IoT Events changing a tracker's state into
"not-responding" from any other state.  On entering this state, it does the following:

- Updates tracker's Shadow document in AWS IoT Core with the new state: not-responding.
...
def lambda_handler(event, context):
    ...
    SUBJECT = event["payload"]["detector"]["keyValue"] + " is not responding"
    BODY_TEXT = (
        "Response not received from tracker " + event["payload"]["detector"]["keyValue"]
    )
    BODY_HTML = """<html>
    <head></head>
    <body>"""
    event["payload"]["detector"]["keyValue"] + ": responses are not being received from the tracker""""
    </body>
    </html>
    """
    CHARSET = "UTF-8"
    ...
    _data = {"state": {"reported": {"status": "not-responding"}}}
    try:
